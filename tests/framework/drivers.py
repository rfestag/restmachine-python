...
class AwsLambdaDriver(DriverInterface):
    ...
    def _convert_to_aws_event(self, request: HttpRequest) -> Dict[str, Any]:
        """Convert DSL HttpRequest to AWS API Gateway event."""
        # Prepare body
        body = None
        ...
        # For simplicity, assume path params are provided separately if needed
    ...
    def _convert_from_aws_response(self, aws_response: Dict[str, Any]) -> HttpResponse:
        """Convert AWS API Gateway response to DSL HttpResponse."""
...
class EnhancedAwsDriver(DriverInterface):
    """
    Enhanced AWS driver with detailed testing capabilities.

    Provides more control over AWS event generation and response handling
    for comprehensive AWS Lambda testing scenarios.
    """
    ...
    def execute(self, request: HttpRequest) -> HttpResponse:
        """Execute request through enhanced AWS API Gateway simulation."""
        # Convert DSL request to AWS API Gateway event
        event = self._convert_to_aws_event(request)
    ...
    def _convert_to_aws_event(self, request: HttpRequest) -> Dict[str, Any]:
        """Convert DSL HttpRequest to AWS API Gateway event with enhanced options."""
        # Prepare body
        body = None
        is_base64_encoded = False
