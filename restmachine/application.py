            ...
            for param_name, param in sig.parameters.items():
                ...
                if param_name in self._validation_dependencies:
                    ...
                    if hasattr(validation_wrapper, 'depends_on_body') and validation_wrapper.depends_on_body:
                        ...
                        if return_annotation and return_annotation != inspect.Parameter.empty:
                            if self._is_pydantic_model(return_annotation):
                                return self._get_pydantic_schema(return_annotation)

            return None

        def _extract_response_schema(route: RouteHandler) -> Dict[str, Any]:
            """Extract response schema from handler return type annotation."""
            sig = inspect.signature(route.handler)
            ...
            if return_annotation and return_annotation != inspect.Parameter.empty:
                ...
                if origin is list:
                    ...
                    if args and self._is_pydantic_model(args[0]):
                        item_schema = self._get_pydantic_schema(args[0])
                        return {
                            "type": "array",
                            "items": item_schema
                        }
                elif self._is_pydantic_model(actual_type):
                    return self._get_pydantic_schema(actual_type)
        ...
        def _get_operation_info(route: RouteHandler) -> Dict[str, Any]:
            ...
            if route.method.value in ["POST", "PUT", "PATCH"]:
                ...
                if request_body_schema:
                    ...
                    # Fall back to generic object schema
                    operation["requestBody"] = {
                        "content": {
                            "application/json": {
                                "schema": {"type": "object"}
