...
def post_service(database_connection, user_service):
    ...
    def get_posts_by_author(author_id: str) -> list:
        return [post for post in database_connection["posts"].values() 
...
def auth_service():
    ...
    return {
        ...
        "get_user": get_user_from_session
    }
...
def login(auth_service, user_service, request: Request):
    ...
    try:
        ...
        return Response(400, '{"error": "Invalid JSON"}', content_type="application/json")

@app.post('/register')
def register(user_service, request: Request):
    """User registration endpoint."""
    if not request.body:
        return Response(400, '{"error": "User data required"}', content_type="application/json")
    ...
    try:
        ...
        for field in required_fields:
            if field not in data:
                return Response(400, f'{{"error": "Missing field: {field}"}}',
                               content_type="application/json")
...
def create_post(post_service, request: Request):
    """Create a new post."""
    if not request.body:
        return Response(400, '{"error": "Post data required"}', content_type="application/json")
    
    try:
        data = json.loads(request.body)
        
        if "title" not in data or "content" not in data:
            return Response(400, '{"error": "Title and content required"}',
                           content_type="application/json")
        ...
        return Response(400, f'{{"error": "{str(e)}"}}', content_type="application/json")
    except json.JSONDecodeError:
        return Response(400, '{"error": "Invalid JSON"}', content_type="application/json")
